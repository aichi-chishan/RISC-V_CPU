...
`define RVC_DECINFO_SYS_MRET_LSB   (`RVC_DECINFO_SYS_FENCE_MSB + 1)
`define RVC_DECINFO_SYS_MRET_MSB   (`RVC_DECINFO_SYS_MRET_LSB)
`define RVC_DECINFO_SYS_MRET       `RVC_DECINFO_SYS_MRET_MSB : `RVC_DECINFO_SYS_MRET_LSB

`define RVC_DECINFO_SYS_CSR_LSB    (`RVC_DECINFO_SYS_MRET_MSB + 1)
`define RVC_DECINFO_SYS_CSR_MSB    (`RVC_DECINFO_SYS_CSR_LSB)
`define RVC_DECINFO_SYS_CSR        `RVC_DECINFO_SYS_CSR_MSB : `RVC_DECINFO_SYS_CSR_LSB

// CSR 命令沿用 funct3 低两位：01=写，10=置位，11=清位；IMM 表示源为 zimm。
`define RVC_DECINFO_SYS_CSR_CMD_LSB (`RVC_DECINFO_SYS_CSR_MSB + 1)
`define RVC_DECINFO_SYS_CSR_CMD_MSB (`RVC_DECINFO_SYS_CSR_CMD_LSB + 1)
`define RVC_DECINFO_SYS_CSR_CMD     `RVC_DECINFO_SYS_CSR_CMD_MSB : `RVC_DECINFO_SYS_CSR_CMD_LSB
`define RVC_DECINFO_SYS_CSR_IMM_LSB (`RVC_DECINFO_SYS_CSR_CMD_MSB + 1)
`define RVC_DECINFO_SYS_CSR_IMM_MSB (`RVC_DECINFO_SYS_CSR_IMM_LSB)
`define RVC_DECINFO_SYS_CSR_IMM     `RVC_DECINFO_SYS_CSR_IMM_MSB : `RVC_DECINFO_SYS_CSR_IMM_LSB
...
`define RVC_CAUSE_ILLEGAL       32'd2
`define RVC_CAUSE_BREAKPOINT    32'd3
`define RVC_CAUSE_LOAD_MISALIGN 32'd4
`define RVC_CAUSE_LOAD_ACCESS   32'd5
`define RVC_CAUSE_STORE_MISALIGN 32'd6
`define RVC_CAUSE_STORE_ACCESS  32'd7
`define RVC_CAUSE_ECALL_M       32'd11
